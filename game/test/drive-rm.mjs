/**
 * Card-payment smoke (Revenue Monster, MYR): the store's coin packs → RM
 * checkout opened in the browser → "waiting" sheet → coins credited; and the
 * trip back from RM's page (startapp=store_<orderId>) landing on the store,
...
try {
  ...
  check((await page.locator('#store-note').innerText()).includes('card or e-wallet'), 'the note says how it is paid');
  ...
  check(JSON.stringify(state.orderBodies) === JSON.stringify([{ packId: 'coins-100' }]), 'only the pack id is sent: the server prices it');
  ...
  /* ---------- in Telegram with Stars on too: two prices per pack, 320 wide ---------- */
  ...
  // Just enough of Telegram's WebApp for canPayInvoices(); every other SDK call is a no-op.
  await page4.addInitScript(() => {
    const noop = () => {};
    window.Telegram = {
      WebApp: new Proxy({
        initData: 'stub', initDataUnsafe: {}, version: '8.0', platform: 'android', themeParams: {},
        isVersionAtLeast: () => true, openInvoice: noop, openLink: noop,
  ...
  // The real SDK script would replace the stand-in: serve nothing in its place.
  await page4.route('https://telegram.org/js/telegram-web-app.js', (route) => route.fulfill({ contentType: 'text/javascript', body: '' }));
  await stubApi(page4, newState({ starsEnabled: true }));
