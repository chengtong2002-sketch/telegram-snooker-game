/**
 * Coin packs on sale: how many coins, and the price through each provider.
 *
 ...
 * sends one. A pack with no price for a provider is not offered through it.
 *
...
export const DEFAULT_PACKS = Object.freeze([
  Object.freeze({ id: 'coins-100', coins: 100, stars: 100, myrSen: 490 }),
  Object.freeze({ id: 'coins-550', coins: 550, stars: 400, myrSen: 1990 }),
  Object.freeze({ id: 'coins-1200', coins: 1200, stars: 800, myrSen: 3990 }),
]);
...
export function parsePacks(raw) {
  ...
  return Object.freeze(list.map((p, i) => {
    ...
    if (!priceOrNull(p.stars)) throw new Error(`${where}.stars must be a positive whole number or null`);
    if (!priceOrNull(p.myrSen)) throw new Error(`${where}.myrSen must be a positive whole number of sen or null`);
    return Object.freeze({ id: p.id, coins: p.coins, stars: p.stars ?? null, myrSen: p.myrSen ?? null });
  }));
}
