 ...
 *          point has dropped
 *
 * r must be large enough to swallow a ball resting against the very end of a
 * cushion, or balls wedge in the jaws instead of dropping. Corner: a ball on
 * the rail at the cushion end sits hypot(CUT, BALL_RADIUS) ≈ 7.0 from the
 * point, so r = 8.0 clears it. Middle: hypot(5.5, 2.0 + 2.625) ≈ 7.2, so
 * r = 7.4 clears it while keeping the jaw about 5.8cm each side of centre.
 */
...
export const POCKETS = [
  ...
  { id: 'tm', x: TABLE.width / 2,  y: -2.0,               r: 7.4, type: 'middle' },
  { id: 'tr', x: TABLE.width,      y: 0,                  r: 8.0, type: 'corner' },
  { id: 'bl', x: 0,                y: TABLE.height,       r: 8.0, type: 'corner' },
  { id: 'bm', x: TABLE.width / 2,  y: TABLE.height + 2.0, r: 7.4, type: 'middle' },
